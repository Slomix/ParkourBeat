...
public class Heads {
    ...
    private static final Material HEAD_MATERIAL = Material.getMaterial(USE_MODERN_HEADS ? "PLAYER_HEAD" : "SKULL_ITEM");
    ...
    @Nullable
    public static String getProfileSkinHash(PlayerProfile profile) {
        String url = getProfileSkinUrl(profile);
        return url == null ? null : url.substring(url.lastIndexOf("/") + 1);
    }

    @Nullable
    public static String getProfileSkinUrl(PlayerProfile profile) {
